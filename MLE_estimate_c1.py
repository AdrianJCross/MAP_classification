...
#plot 3D gaussian
z0=np.array([gaussian_pdf(np.array([x,y]).reshape(-1,1),mu_c1,sigma_c1)
            for x,y in zip(np.ravel(X), np.ravel(Y))])
Z0=z0.reshape(X.shape)
fig=plt.figure()
ax=fig.gca(projection='3d')
ax.plot_surface(X,Y,Z0,cmap=plt.cm.viridis)

c_0_set=ax.contourf(X, Y, Z0, zdir='z',cmap=plt.cm.viridis)
plt.xlabel('x')
plt.ylabel('y')
plt.title('Class 1 Gaussian contour plot')
plt.show()
